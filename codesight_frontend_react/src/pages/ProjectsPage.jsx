      ...
      <div className="table-card">
        <div className="table-toolbar">
          ...
          <div className="toolbar-actions">
            <span className="chip">Все статусы</span>
            <span className="chip">Все источники</span>
          </div>
        </div>
          ...
          <table>
            ...
            <tbody>
              {filteredProjects.map((project) => {
                const status = STATUS_LABEL[project.status] || { label: project.status || '—', cls: 'pill-neutral' };
                return (
                    ...
                    <td>
                      <div className="project-cell">
                        ...
                        <div>
                          ...
                          <div className="project-meta">{project.repo_url || 'ZIP-архив'}</div>
                        </div>
                      </div>
                    </td>
                    <td><span className={`pill ${status.cls}`}>{status.label}</span></td>
                    <td style={{ color: 'var(--muted)', fontSize: 13 }}>{project.repo_url ? 'Git URL' : 'ZIP'}</td>
                    <td style={{ color: 'var(--muted)', fontSize: 13 }}>{project.created_at ? new Date(project.created_at).toLocaleDateString('ru-RU') : '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button className="btn btn-ghost" style={{ padding: '7px 10px' }} onClick={(e) => { e.stopPropagation(); navigate(`/projects/${project.id}`); }} title="Открыть">
                          <Eye size={15} />
                        </button>
                        <button className="btn btn-danger" style={{ padding: '7px 10px' }} onClick={(e) => handleDelete(project.id, e)} title="Удалить">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </td>
