"""Обработка команд Kafka (шаг security)."""
...
from .service import _execute_scan
...
async def _handle_command(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        await _execute_scan(run_id, project_id)

        async with AsyncSessionLocal() as db:
            scan = await db.get(SecurityScan, run_id)
        if scan is None:
            err = "scan record lost"
        else:
            status = "completed" if scan.status == "completed" else "failed"
            err = scan.error_message if status == "failed" else None
    except Exception as exc:  # noqa: BLE001
        logger.exception("Ошибка security по Kafka")
        err = str(exc)
